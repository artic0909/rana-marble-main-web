        ...
        <div class="detail-grid">
            ...
            <div class="gallery-col">
                <div class="main-image-wrap" id="mainImgWrap">
                    ...
                    @guest
                        <a href="{{route('login')}}" class="wish-main" id="wishMainBtn" title="Add to Wishlist"
                    ...
                    @auth
                        <button class="wish-main" id="wishMainBtn"
                            onclick="event.stopPropagation(); toggleWishlist(this, {{ $product->id }})" title="Add to Wishlist"
                    ...
                    <div class="gallery-slider" id="gallerySlider">
                        ...
                        @if($product->main_image)
                            <div class="gallery-slide" data-index="0"
                                onclick="openLightbox('{{ Storage::url($product->main_image) }}', 'image')">
                                <img src="{{ Storage::url($product->main_image) }}" alt="{{ $product->name }}">
                            </div>
                        @endif

                        {{-- Gallery slides --}}
                        @php $slideIdx = 1; @endphp
                        @foreach($product->images as $media)
                            <div class="gallery-slide" data-index="{{ $slideIdx }}"
                                onclick="openLightbox('{{ Storage::url($media->image) }}', '{{ $media->type }}')">
                                @if($media->type === 'video')
                                    <video src="{{ Storage::url($media->image) }}" controls playsinline></video>
                                @else
                ...
                <div class="thumbnails">
                    ...
                    @if($product->main_image)
                        <div class="thumb active" onclick="switchMedia(this, 0)">
                            <img src="{{ Storage::url($product->main_image) }}" alt="{{ $product->name }}" />
                        </div>
                    @endif

                    {{-- Gallery images & videos --}}
                    @php $thumbIdx = 1; @endphp
                    @foreach($product->images as $media)
                        @if($media->type === 'video')
                            <div class="thumb video-thumb" onclick="switchMedia(this, {{ $thumbIdx }})">
                                {{-- Use a video element with preload to grab first frame as poster --}}
                                <video src="{{ Storage::url($media->image) }}"
                        ...
                        @else
                            <div class="thumb" onclick="switchMedia(this, {{ $thumbIdx }})">
                                <img src="{{ Storage::url($media->image) }}" alt="{{ $product->name }}" />
                            </div>
                        @endif
                        @php $thumbIdx++; @endphp
            ...
            <div class="info-col">
                <div class="product-tag-row">
                    <span class="ptag cat"><i class="fas fa-place-of-worship"></i>
                    ...
                    <div class="variant-section">
                        ...
                        <div class="size-options">
                            @foreach($uniqueSizes as $i => $variant)
                                <div class="size-opt {{ $i === 0 ? 'active' : '' }}" data-size-id="{{ $variant->size_id }}"
                ...
                <div class="write-review-section">
                    ...
                    <form class="write-review-form" action="{{ route('reviews.store', $product->id) }}" method="POST"
                        ...
                        <div class="review-form-fields">
                            <div class="rf-row">
                                <div class="rf-col">
                                    ...
                                    <input type="text" name="name" placeholder="e.g. Ramesh Gupta" required
                            ...
                            <div class="form-group review-media-upload" style="grid-column: 1 / -1; margin-top: 10px;">
                                <label for="reviewMedia" class="r-upload-btn" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 16px;
                        ...
                        @if (!empty($review->media) && count($review->media))
                            <div class="review-media-grid">
                                @foreach ($review->media as $mediaPath)
                                    ...
                                    @if ($isVideo)
                                        <div class="r-media-item video" onclick="openReviewMediaLightbox(this, 'video', '{{ $url }}')">
                                            <video src="{{ $url }}" style="width:100%;height:100%;object-fit:cover;pointer-events:none;"
                                                preload="metadata" muted playsinline></video>
                                            <div class="r-play-btn"><i class="fas fa-play"></i></div>
                                        </div>
                                    @else
                                        <img src="{{ $url }}" class="r-media-item" onclick="openReviewMediaLightbox(this)"
                    ...
                    <div class="product-card" style="background: #1b0b06 !important;">
                        <div class="product-img-wrap">
                            ...
                            <div class="prod-actions">
                                ...
                                @endguest
                                @auth
                                    <button class="action-btn" onclick="toggleWishlist(this, {{ $product->id }})" title="Wishlist"
                                        style="{{ Auth::guard('customer')->check() && $product->wishlists->where('customer_id', Auth::guard('customer')->id())->count() ? 'color:var(--saffron);' : '' }}">
                                        <i class="fas fa-heart"></i>
                                    </button>
                                @endauth
                                <a href="{{ route('product.detail', $product->slug) }}" class="prod-action-btn"><i
